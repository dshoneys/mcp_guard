...
本机代理：对机器上的 **MCP / Agent 工具调用�?* �?**扫描、监视、审�?*（后续再做硬拦截）�?
...
MCP Guard **�?*跑在浏览器里。它作为主机�?Agent，盯住共同瓶颈：**谁能访问本机 MCP 端口**�?
...
�?Agent / O(1) 规则�?[`doc/structurizr/`](doc/structurizr/README.md)�?

- 插件出度 O(1)（`model/graph.json`�?
- \(R_{\mathrm{manual}}=R\setminus R_U\)（`model/requirements.json`）→ 需人测�?
...
2. **归因（`watch`�?*  
   读系�?TCP 表，映射套接�?�?PID �?进程�?路径；对�?`gate.allow_process_names`�? 
   未知客户�?�?**ACTIVITY ALERT** + 审计 `activity_alert`�?
...
| 告警 | 何时 |
|------|------|
| `exposure_alert` | 发现未保护的 MCP 风险�?|
| `activity_alert` | 非白名单进程正在连接该端�?|
...
| 能力 | 状�?|
|------|------|
| `mcp-guard scan` | �?�?MCP �?+ 本机 **反射 XSS** canary（`xss_reflected_unescaped`�?|
| `mcp-guard watch` | �?软归�?|
| `mcp-guard serve` | �?scan + watch + 审计 |
| `mcp-guard tray` | �?托盘 + 主界�?+ 后台 Agent�?*默认中文**；`--locale en`�?|
| `mcp-guard dashboard` | �?仅主窗口（无托盘；日常用 `tray`�?|
| `mcp-guard status` | �?菜单模型 + 审计快照 JSON |
| `mcp-guard vault` / `vault-mcp` | �?NoContext 密钥保险�?|
| `mcp-guard git-scan` | �?本地 git �?reasoning 密文入库（[arXiv:2608.09867](https://arxiv.org/abs/2608.09867)�?|
| `web/xss-reflect-scan` | �?�?JS 同语义探针（前端静态部�?/ [`demo.html`](web/xss-reflect-scan/demo.html)�?|
| 硬端�?进程拦截 | �?|
| 路径 / 工具策略 | �?|
...
厂商�?CoT 做成不透明 AEAD 块；一旦写�?git，公开仓库可被回放解密。见攻防 case：[`cases/arxiv-2608-09867/`](cases/arxiv-2608-09867/)�?
