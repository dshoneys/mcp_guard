#!/usr/bin/env python3
"""ADL gate: O(1) plugins + R_manual + UX→impl→UI design gate.

Usage:
  python scripts/adl_check.py
  python scripts/adl_check.py --root .
"""
...
# Preview is the strict Code-as-Design source (no Figma)
...
def check_requirements(reqs: dict, root: Path) -> tuple[list[str], dict]:
    ...
    for r in R:
        ...
        if "design_status" in r or "design_path" in r:
            errors.append(
                f"{rid}: deprecated design_path/design_status — use ux_* and ui_* (see UI-DESIGN.md)"
            )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="MCP Guard ADL checker")
    ap.add_argument(
        "--root",
        type=Path,
    ...
    if hollow:
        ...
        print("[OK] O(1) + requirements + UX/UI gates satisfied")
    print("     Implement only after UX accepted; skin after UI pack / ui_impl config|code.")
    return 0
